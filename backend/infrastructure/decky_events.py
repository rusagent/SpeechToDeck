...
same module object as `decky_plugin`. One emit writes one JSON socket line
that the frontend dispatches to this plugin's listeners as
`listener(...args)`; our single-dict payloads arrive as `args[0]`, matching
the frontend `DeckyBackendClient`.
"""
...
# Containment: a failing emit must never crash the plugin (an event is
...
class DeckyEventPublisher:
    ...
    payload text, payloads may carry transcripts — and swallowed. No
    ...
    async def publish(self, event_name: str, payload: dict[str, object]) -> None:
        try:
            ...
            # payload (transcript text) and never the exception body,
