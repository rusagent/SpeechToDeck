...
def build_fixture_binary(
    ...
    *,
    ...
    """Write an executable `bin/voxtype` launcher for the fixture daemon.

    `extra_daemon_args` are appended to every `daemon` invocation, which lets
    a test script crash loops, SIGTERM-ignoring or grandchild behaviour while
    the supervisor still hashes and runs exactly `bin/voxtype`.
    """
    bin_dir = plugin_root / "bin"
    bin_dir.mkdir(parents=True, exist_ok=True)
    binary = bin_dir / "voxtype"
    real = plugin_root / "bin" / ".voxtype-fake.py"
...
def write_pinned_runtime_manifest(
    plugin_root: Path, binary: Path | None = None, *, digest: str | None = None
) -> Path:
    """Pin defaults/runtime-manifest.json to the exact fixture binary bytes.

    `digest` overrides hashing (used when the binary is deliberately absent).
    """
    if digest is None:
                ...
                "artifacts": [
                    {
                        ...
                        "arch": "x86_64",
                        "version": "v9.9.9-test",
                        "source": "https://example.test/voxtype/releases/v9.9.9",
                        "sha256": digest,
                        "license": "MIT",
                    }
                ],
...
async def spawn_fixture_daemon(
    paths: PluginPaths, binary: Path, **extra_args: str
    ...
    if not await wait_until(lambda: paths.control_socket.exists(), timeout=3.0):
        proc.kill()
        await proc.wait()
        pytest.fail("fixture daemon did not open its control socket")
    return proc
