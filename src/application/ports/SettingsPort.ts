/**
 * Settings access port. The backend owns persistence; the frontend loads
 * settings through a Decky callable and never writes settings files directly.
 */

/**
 * Model id format of the backend settings validator
 * (json_settings_repository._MODEL_ID_RE): any curated-catalog id matches;
 * malformed ids are rejected before they are saved.
 */
...
export interface PluginSettings {
    ...
    /** A curated-catalog model id; format-checked by the guard. */
    modelId: string;
...
/**
 * Manual type guard for settings crossing the backend boundary.
